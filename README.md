...
- 🎯 Entry points to execute your program using `python -m <fast_app>` or `$ fast_app` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/Harnath-Eunimart/fast_app/branch/main/graph/badge.svg?token=fast_app_token_here)](https://codecov.io/gh/Harnath-Eunimart/fast_app)
[![CI](https://github.com/Harnath-Eunimart/fast_app/actions/workflows/main.yml/badge.svg)](https://github.com/Harnath-Eunimart/fast_app/actions/workflows/main.yml)

Awesome fast_app created by Harnath-Eunimart

## Install

from source
```bash
git clone https://github.com/Harnath-Eunimart/fast_app fast_app
cd fast_app
make install
```
...
```bash
pip install fast_app
```

## Executing

```bash
$ fast_app run --port 8080
```

or

```bash
python -m fast_app run --port 8080
```
...
```bash
❯ fast_app --help
Usage: fast_app [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ fast_app create-user --help
Usage: fast_app create-user [OPTIONS] USERNAME PASSWORD
...
```bash
fast_app create-user admin admin --superuser
```
...
Run with `fast_app run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `fast_app create-user`.
...
-----------------------------------------------------
fast_app/__init__.py              4      0   100%
fast_app/app.py                  16      1    94%
fast_app/cli.py                  21      0   100%
fast_app/config.py                5      0   100%
fast_app/db.py                   10      0   100%
fast_app/models/__init__.py       0      0   100%
fast_app/models/content.py       47      1    98%
fast_app/routes/__init__.py      11      0   100%
fast_app/routes/content.py       52     25    52%
fast_app/routes/security.py      15      1    93%
fast_app/routes/user.py          52     26    50%
fast_app/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in fast_app/default.toml.

### As environment variables
```bash
export fast_app_KEY=value
export fast_app_KEY="@int 42"
export fast_app_KEY="@jinja {{ this.db.uri }}"
export fast_app_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
fast_app_ENV=production fast_app run
```
